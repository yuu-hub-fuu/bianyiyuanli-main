...
> GUI 口径修正：当前恢复的 IDE 是 `nexa/ide/app.py` 中的 Python tkinter 桌面图形化程序，不是 FastAPI/WebSocket 网页端，也不是 PySide6。它支持源码编辑、Token、AST、符号表、HIR、CFG、ASM、LLVM、Timeline、Run、Trace、诊断、快速修复和 HTML 报告导出。
Block        ::= "{" { Stmt } "}"
